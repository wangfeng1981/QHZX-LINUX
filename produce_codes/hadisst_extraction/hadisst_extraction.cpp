...
{
	std::ofstream datofs(datfilepath.c_str());
...
int main(int argc  , char** argv )
{
	...
	std::ofstream plotfs(plotfile.c_str());
	...
	if (ifs.is_open())
	{
		...
		while (std::getline(ifs, line)) {
			...
			if (line.length() < 100)
			{
				...
				{
					assert(irows == 180);
					dsPtr->GetRasterBand(1)->RasterIO( GF_Write, 0, 0, 360, 180, buff, 360, 180, GDT_Int32, 0, 0, 0);
					GDALClose(dsPtr);
					dsPtr = 0;
				...
				irows = 0;
				dsPtr = tifDriver->Create(outfilepath.c_str(), 360, 180, 1, GDT_Int16, 0);
