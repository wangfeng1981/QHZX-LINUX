...
int main(int argc , char** argv )
{
	...
	GDALDataset* productDataset = (GDALDataset*)GDALOpen(productfile.c_str(), GA_ReadOnly);
	...
	if (appMode == 0)
	{
		GDALDataset* lonDataset = (GDALDataset*)GDALOpen(lonfile.c_str(), GA_ReadOnly);
		GDALDataset* latDataset = (GDALDataset*)GDALOpen(latfile.c_str(), GA_ReadOnly);
		...
		for (int iy = 0; iy < rasterYSize; ++iy)
		{
			...
			lonBand->RasterIO(GF_Write, 0, iy, rasterXSize, 1, lonValuesRow, rasterXSize, 1, GDT_Float32, 0, 0);
			latBand->RasterIO(GF_Write, 0, iy, rasterXSize, 1, latValuesRow, rasterXSize, 1, GDT_Float32, 0, 0);
		}
		delete[] lonValuesRow;
		delete[] latValuesRow;
