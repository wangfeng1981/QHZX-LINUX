...
extern int processOneFile(string undbFile, string fy4totxtProgram, string fy4lonFile, string fy4latFile, 
	string logDir, string programid,string	plotTemFile , string vrtProgram , string gdalwarp , string plotProgram );
...
int main( int argc  , char** argv )
{
	...
	string inDir, outDir, plotTemFile , logDir , 
		...
		vrtProgram , gdalwarpProgram , plotProgram ;
	...
	gdalwarpProgram = wft_getValueFromExtraParamsFile(startupFile, "#gdalwarpprogram", true); 
	plotProgram = wft_getValueFromExtraParamsFile(startupFile, "#plotprogram", true); 
	int maxError = 2;
	...
	for (size_t i = 0; i < dirFiles.size(); ++i)
	{
		...
		{//ÓÐÐ§µÄ·çËÄsstÊý¾Ý
			...
			{
				undbFile = tempfile;
				int ret = processOneFile(undbFile, fy4totxtProgram, fy4lonFile, fy4latFile, logDir, programid, plotTemFile , 
					vrtProgram , gdalwarpProgram , plotProgram );
				if (ret == 100) {
					break;
				}
...
int processOneFile(string undbFile , string fy4totxtProgram , string fy4lonFile , string fy4latFile ,
	...
	string vrtProgram , string gdalwarpProgram , string plotProgram )
{
	...
	string llvFile = undbFile + ".llv.txt";
	{
		...
		string cmd22 = gdalwarpProgram + " -geoloc -t_srs \"+proj=longlat +ellps=WGS84\" -r bilinear -tr 0.05 0.05 -overwrite ";
		...
		if (wft_test_file_exists(wgs84tif) == false) {
			wft_log(logDir , programid , cmd22 + " cmd is failed to call. out.") ;
