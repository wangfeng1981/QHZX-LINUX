...
int main(int argc , char** argv )
{
	...
	GDALDataset* baseDataDs = (GDALDataset*)GDALOpen(inputbaseDataPath.c_str(),  GA_ReadOnly);
	GDALDataset* baseLonDs = (GDALDataset*)GDALOpen(inputbaseLonPath.c_str(),  GA_ReadOnly);
	GDALDataset* baseLatDs = (GDALDataset*)GDALOpen(inputbaseLatPath.c_str(),  GA_ReadOnly);

	GDALDataset* refDataDs = (GDALDataset*)GDALOpen(inputrefDataPath.c_str(),  GA_ReadOnly);
	...
	GDALDataset* outputBiasDataset = driver->Create(outputbiasfile.c_str(), baseXSize, baseYSize , 1,  GDT_Float32, nullptr);
	...
	for (int iby = 0; iby < baseYSize; ++iby)
	{
		...
		baseDataDs->GetRasterBand(1)->RasterIO( GF_Read, 0, iby, baseXSize, 1,
			dataValues, baseXSize, 1,  GDT_Float32, 0, 0, nullptr);
		baseLonDs->GetRasterBand(1)->RasterIO( GF_Read, 0, iby, baseXSize, 1,
			lonValues, baseXSize, 1,  GDT_Float32, 0, 0, nullptr);
		baseLatDs->GetRasterBand(1)->RasterIO( GF_Read, 0, iby, baseXSize, 1,
			latValues, baseXSize, 1,  GDT_Float32, 0, 0, nullptr);
		...
		for (int ibx = 0; ibx < baseXSize; ++ibx)
		{
			...
			if (baseValue >= inputbaseDataValid0 && baseValue <= inputbaseDataValid1 && blonValue>-998 && blatValue>-998 )
			{
				...
				//		refDataDs->GetRasterBand(1)->RasterIO( GF_Read, 0, targetRefY, refXSize, copyedLines,
				//			refDataBuffer, refXSize, copyedLines,  GDT_Float32, 0, 0, nullptr);
				...
				{//4
					...
					refDataDs->GetRasterBand(1)->RasterIO( GF_Read, targetRefX, targetRefY,2,2,
						tempRefValueArr, 2, 2,  GDT_Float32, 0, 0, nullptr);
				...
				{//2
					...
					refDataDs->GetRasterBand(1)->RasterIO( GF_Read, targetRefX, targetRefY, 1, 2,
						tempRefValueArr, 1, 2,  GDT_Float32, 0, 0, nullptr);
					//tempRefValueArr[0] = refDataBuffer[(targetRefY - refBufferY0Index)*refXSize + targetRefX];//缓存版本
				...
				{//2
					float tempRefValueArr[2];
					refDataDs->GetRasterBand(1)->RasterIO( GF_Read, targetRefX, targetRefY, 2, 1,
						tempRefValueArr, 2, 1,  GDT_Float32, 0, 0, nullptr);
					//tempRefValueArr[0] = refDataBuffer[(targetRefY - refBufferY0Index)*refXSize + targetRefX];//缓存版本
					//tempRefValueArr[1] = refDataBuffer[(targetRefY - refBufferY0Index)*refXSize + targetRefX+1];//缓存版本
					if (inputrefDataValid0 <= tempRefValueArr[0] && tempRefValueArr[0] <= inputrefDataValid1)
					{
				...
				{//1
					...
					refDataDs->GetRasterBand(1)->RasterIO( GF_Read, targetRefX, targetRefY, 1, 1,
						&tempRefValue, 1, 1,  GDT_Float32, 0, 0, nullptr);
		...
		outputBiasDataset->GetRasterBand(1)->RasterIO( GF_Write, 0, iby, baseXSize, 1, 
				outBiasRowValues , baseXSize , 1,  GDT_Float32, 0, 0, nullptr);
		//progress
		{
	...
	//生成绘图gnuplot文件
	{
		...
		plotOfs << "reset" << std::endl;
		plotOfs << "set terminal png size 1000,800" << std::endl;
