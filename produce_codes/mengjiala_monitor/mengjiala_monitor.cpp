...
bool isvalidFy4SstDailyFile(string& filename)
{
	...
	size_t pos0 = filename.find("FY4A-_AGRI--_N_DISK_1047E_L2-_SST-_MULT_NOM_");
...
void extractSst90EData(string& fy4sstfilepath, string& sst90efilepath)
{
	cout<<fy4sstfilepath<<endl ;
	GDALDataset* fy4sst = (GDALDataset*)GDALOpen(fy4sstfilepath.c_str(), GA_ReadOnly);
	int xsize = fy4sst->GetRasterXSize();
	int ysize = fy4sst->GetRasterYSize();
	float* buffer = new float[xsize * ysize];
	if( buffer == 0 )
	{
		cout<<"Error : failed to alloc memory of "<<xsize<<" x "<<ysize<<endl ;
		exit(110) ;
	}
	fy4sst->GetRasterBand(1)->RasterIO(GF_Read, 0, 0, xsize, ysize,
		buffer, xsize, ysize, GDT_Float32, 0, 0, 0);
	ofstream ofs(sst90efilepath.c_str());
	ofs << "#lat sst for mengjiala 90E" << endl;
	for (int i = 0; i < fy4_90e_num; ++i)
	{
		float sst = buffer[fy490eindex[i]];
		float lat = fy490elat[i];
		if (sst > -10 && sst < 60)
		{
			ofs << lat << "\t" << sst << endl;
		}
...
void str2tm(string ymd, struct tm * t)
{
	...
	t->tm_mday = (int)atof(day.c_str());
	t->tm_mon = (int)atof(month.c_str()) - 1;
	t->tm_year = (int)atof(year.c_str()) - 1900;
}
