...
{
	std::cout << "Version 0.1.2a." << endl;
	std::cout << "Version 0.2a. can set output rect by pixels coordinate." << endl;
	std::cout<<"Version 0.2a for linux. modified for linux. 2017-10-20."<<endl;
	std::cout<<"Version 0.2a for linux bugfixed for use x range 2017-10-25."<<endl;
	...
	GDALDataset* lonDs = 0;
	GDALDataset* latDs = 0;
	inputDs = (GDALDataset*)GDALOpen(inputDatasetpath.c_str(), GA_ReadOnly);
	if (inputDs == 0)
	{
	...
	if (llType == "llfiles")
	{
		lonDs = (GDALDataset*)GDALOpen(lonFile.c_str(), GA_ReadOnly);
		latDs = (GDALDataset*)GDALOpen(latFile.c_str(), GA_ReadOnly);
		if (lonDs == 0 || latDs == 0)
		{
	...
	for (int iy = 0; iy < ySize; ++iy)
	{
		...
		inputDs->GetRasterBand(1)->RasterIO(GF_Read, 0, iy,
											xSize, 1,buffer, xSize, 1, GDT_Float64, 0, 0);
		if( lonbuffer ){
			lonDs->GetRasterBand(1)->RasterIO(GF_Read, 0, iy,
				xSize, 1, lonbuffer, xSize, 1, GDT_Float64, 0, 0);
			latDs->GetRasterBand(1)->RasterIO(GF_Read, 0, iy,
				xSize, 1, latbuffer, xSize, 1, GDT_Float64, 0, 0);
		}
